...
BOOST_AUTO_TEST_CASE(data_term_test) {
	...
	// @formatter:on
	eig::MatrixXf live_gradient_x_field(4, 4);
	...
	// @formatter:on
	eig::MatrixXf live_gradient_y_field(4, 4);
...
BOOST_AUTO_TEST_CASE(warp_field_test02) {
	eig::MatrixXf warped_live_field(3, 3), canonical_field(3, 3);
	eig::MatrixXf u_vectors(3, 3), v_vectors(3, 3);
	// @formatter:off
	...
	eig::Matrix3f expected_live_out;
	// @formatter:off
	...
	eig::Matrix3f warped_live_field_out = nonrigid_optimization::warp_2d_advanced(warp_field, warped_live_field, canonical_field,
			true, false, true);
	BOOST_REQUIRE(warped_live_field_out.isApprox(expected_live_out));
...
BOOST_AUTO_TEST_CASE(warp_field_test03) {
	eig::MatrixXf warped_live_field(4, 4), canonical_field(4, 4);
	...
	// @formatter:on
	math::MatrixXv2f warp_field = math::stack_as_xv2f(u_vectors, v_vectors);
	eig::MatrixXf warped_live_field_out = nonrigid_optimization::warp_2d_advanced(warp_field, warped_live_field, canonical_field,
			false, false, false);

	eig::MatrixXf expected_live_out(4, 4);
	// @formatter:off
...
BOOST_AUTO_TEST_CASE(warp_field_test04) {
	eig::MatrixXf warped_live_field_out = nonrigid_optimization::warp_2d_advanced(
...
BOOST_AUTO_TEST_CASE(test_sobolev_optimizer02) {
	...
	telemetry::TsdfDifferenceStatistics2d expected_diff_stats =
			telemetry::TsdfDifferenceStatistics2d(0, 0.246834, 0.111843, 0.0812234, math::Vector2i(3, 3));

	telemetry::ConvergenceReport2d expected_report(2, true, expected_warp_stats, expected_diff_stats);

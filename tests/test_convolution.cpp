...
//test data
#include "data/test_data_convolution.hpp"
...
BOOST_AUTO_TEST_CASE(test_convolve_with_kernel_preserve_zeros01) {
...
BOOST_AUTO_TEST_CASE(test_convolve_with_kernel_preserve_zeros02) {
...
BOOST_AUTO_TEST_CASE(test_convolve_with_kernel_y_matrix) {
	math::MatrixXv2f vector_field(4, 4);
	vector_field <<
			math::Vector2f(0.f, 0.f),
			math::Vector2f(0.f, 0.f),
...
BOOST_AUTO_TEST_CASE(test_convolve_with_kernel_x_matrix) {
	math::MatrixXv2f vector_field(4, 4);
	vector_field <<
			math::Vector2f(0.0f, 0.0f),
			math::Vector2f(-0.02738971f, -0.02738965f),
...
BOOST_AUTO_TEST_CASE(test_convolve_with_kernel_matrix) {
	math::MatrixXv2f vector_field(4, 4);
	vector_field <<
			v2(0.f, 0.f), v2(0.f, 0.f), v2(-0.35937524f, -0.18750024f), v2(-0.13125f, -0.17500037f),
			v2(0.f, 0.f), v2(-0.4062504f, -0.4062496f), v2(-0.09375f, -0.1874992f), v2(-0.04375001f, -0.17499907f),
			v2(0.f, 0.f), v2(-0.65624946f, -0.21874908f), v2(-0.09375f, -0.1499992f), v2(-0.04375001f, -0.21874908f),
			v2(0.f, 0.f), v2(-0.5312497f, -0.18750025f), v2(-0.09374999f, -0.15000032f), v2(-0.13125001f, -0.2625004f);
	eig::VectorXf kernel(3);
	kernel << 0.06742075, 0.99544406, 0.06742075;
	math::MatrixXv2f expected_output(4, 4);
	expected_output <<
			math::Vector2f(-0.00184663f, -0.00184663f),
			math::Vector2f(-0.05181003f, -0.04070097f),
			math::Vector2f(-0.37325418f, -0.2127664f),
			math::Vector2f(-0.1575381f, -0.19859035f),
	...
	for (int z = 0; z < vector_field.dimension(2); z++) {
		for (int y = 0; y < vector_field.dimension(1); y++) {
			for (int x = 0; x < vector_field.dimension(0); x++) {
				vector_field(x, y, z) = v3(counter, counter + 1.0f, counter + 2.0f);
				counter += 3.0f;
			}
		}
	}
...
BOOST_AUTO_TEST_CASE(test_convolve_with_kernel_tensor) {
	math::Tensor3v3f vector_field(4, 4, 5);
	gen_arange_tensor3v3f(vector_field);
	eig::VectorXf kernel(3);
	kernel << 3.0, 2.0, 1.0;
	math::convolve_with_kernel(vector_field, kernel);
	BOOST_REQUIRE(math::almost_equal_verbose(vector_field, test_data::convolved_3d_vector_field, 1e-6));
}

BOOST_AUTO_TEST_CASE(test_convolve_with_kernel_x_tensor) {
	math::Tensor3v3f vector_field(4, 4, 5);
	gen_arange_tensor3v3f(vector_field);
	eig::VectorXf kernel(3);
	kernel << 3.0, 2.0, 1.0;
	math::convolve_with_kernel_x(vector_field, kernel);
	BOOST_REQUIRE(math::almost_equal_verbose(vector_field, test_data::convolved_x_3d_vector_field, 1e-6));

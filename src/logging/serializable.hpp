/**
* \brief Class convenient and duplication-free serialisation and deserialisation of its descendants into JSON,
*  with one function to handle both loading and saving
*
...
protected:
	...
	*/
	...
	typename std::enable_if<!std::is_base_of<Serializable, T>::value
			&& std::is_same<bool, typename std::remove_reference<decltype(std::declval<Serializable>().synch(std::declval<std::string>(), *std::declval<T>()))>::type>::value
	...
	*/
	template<typename T>
	typename std::enable_if<std::is_base_of<Serializable, T>::value, bool>::type
	synch(const std::string& key, T& value) {
	...
	*/
	template<typename T>
	typename std::enable_if<std::is_base_of<Serializable, typename std::remove_reference<decltype(*std::declval<T>())>::type>::value
...
public:
	...
	*/
	inline void save(const std::string& fileName) const {
		...
		const_cast<Serializable*>(this)->saveOrLoad();

...
Pyramid3d::Pyramid3d(eig::Tensor<float, 3> field, int maximum_chunk_size) :
	...
	eig::Tensor<float, 3>& previous_level = field;

	for (int i_level = 1; i_level < level_count; i_level++) {
		eig::Tensor<float, 3> current_level(
...
const eig::Tensor<float, 3>& Pyramid3d::level(int i_level) const {
	return this->levels[i_level];
}

size_t Pyramid3d::level_count() const{
